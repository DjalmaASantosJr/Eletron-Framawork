...
/**
 * Implementa a funcionalidade de arrastar e soltar arquivos na janela
...
dropZone.addEventListener('dragover', (event) => {
  ...
  // Opcional: Adicionar uma classe para feedback visual
...
dropZone.addEventListener('dragleave', (event) => {
  ...
  event.stopPropagation();
  // Opcional: Remover a classe de feedback visual
...
dropZone.addEventListener('drop', async (event) => {
  ...
  if (files.length > 0) {
    const filePath = files[0].path; // Pega o caminho do primeiro arquivo
    // Usa a função existente no main process para carregar a imagem
    // (Isso requer que o main process tenha uma função para carregar por path)
    await window.electronAPI.loadImageByPath(filePath);
  }
});
